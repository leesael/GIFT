...
*
* Description: the proposed TF method which utilizes guided regularizations for interpretable and accurate factorizations. 
* Usage:
*   - make gift
*   - ./gift input.txt mask.txt result/ 3 0.01 10 20 30
*/
		...
		for (iter = 0; iter < rowcount; iter++) {
			...
			for (k = 0; k < column_size; k++) {
				for (l = 0; l < column_size; l++) {
					B[pos] = 0;
					if (k == l){
						B[pos] = lambda*(1-Mask[i*mult+j*Core_dim+k]);
					}
					pos++;
				}
				C[k] = 0;
...
int main(int argc, char* argv[]) {
	...
	else printf("Usage: ./gift [input_tensor_path] [mask_matrix_path] [result_directory_path] [tensor_order] [lambda] [tensor_rank]\n");
	return 0;
